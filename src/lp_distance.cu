...
__global__ void lpDistances(char* const valids, T* const distances, const int npreds, const bool isPanelMode,
                            const double idw, const double missingDistance, const int eacts, const int mnobs,
...
void lpDistances(char* const valids, T* const distances, const int npreds, const bool isDMAE, const bool isPanelMode,
                 const double idw, const double missingDistance, const int eacts, const int mnobs, const T* mData,
                 const int* mPanelIds, const T* mpData, const int* mpPanelIds, const char* mopts,
                 const cudaStream_t stream, const dim3 blocks, const dim3 threads)
{
  if (isDMAE) {
    lpDistances<T, true, 32, BLOCK_DIM_Y><<<blocks, threads, 0, stream>>>(valids, distances, npreds, isPanelMode, idw,
                                                                          missingDistance, eacts, mnobs, mData,
                                                                          mPanelIds, mpData, mpPanelIds, mopts);
  } else {
    lpDistances<T, false, 32, BLOCK_DIM_Y><<<blocks, threads, 0, stream>>>(valids, distances, npreds, isPanelMode, idw,
                                                                           missingDistance, eacts, mnobs, mData,
                                                                           mPanelIds, mpData, mpPanelIds, mopts);
  }
}
...
template<typename T>
void cuLPDistances(char* const valids, T* const distances, const int npreds, const bool isDMAE, const bool isPanelMode,
                   const double idw, const double missingDistance, const int eacts, const int mnobs, const T* mData,
                   const int* mPanelIds, const T* mpData, const int* mpPanelIds, const char* mopts,
                   const cudaStream_t stream)
{
  ...
  switch (threads.y) {
    case 8:
      lpDistances<T, 8>(valids, distances, npreds, isDMAE, isPanelMode, idw, missingDistance, eacts, mnobs, mData,
                        mPanelIds, mpData, mpPanelIds, mopts, stream, blocks, threads);
      break;
    case 4:
      lpDistances<T, 4>(valids, distances, npreds, isDMAE, isPanelMode, idw, missingDistance, eacts, mnobs, mData,
                        mPanelIds, mpData, mpPanelIds, mopts, stream, blocks, threads);
      break;
    default:
      lpDistances<T, 2>(valids, distances, npreds, isDMAE, isPanelMode, idw, missingDistance, eacts, mnobs, mData,
                        mPanelIds, mpData, mpPanelIds, mopts, stream, blocks, threads);
      break;
  }
}
