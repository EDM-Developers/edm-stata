                          ...
                          bool keep_going(), void all_tasks_finished())
{
  ...
  if (keep_going == nullptr || keep_going()) {
    ...
    if (opts.saveManifolds) {
      pred.M = std::make_unique<Manifold>(generator, E, libraryRows, false, opts.dtWeight, opts.copredict, false);
      pred.Mp = std::make_unique<Manifold>(generator, E, predictionRows, true, opts.dtWeight, opts.copredict, false);
    } else {
      pred.M = nullptr;
      pred.Mp = nullptr;
    }
