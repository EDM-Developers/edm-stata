...
public:
  ...
  explicit Client(const char* scheme_host_port, const std::string& client_cert_path,
                  const std::string& client_key_path);
...
template<typename T>
inline bool process_server_socket_core(socket_t sock, size_t keep_alive_max_count, time_t keep_alive_timeout_sec,
                                       T callback)
{
...
template<typename T>
inline bool process_server_socket(socket_t sock, size_t keep_alive_max_count, time_t keep_alive_timeout_sec,
...
inline socket_t create_client_socket(const char* host, int port, bool tcp_nodelay, SocketOptions socket_options,
                                     time_t timeout_sec, time_t timeout_usec, const std::string& intf, Error& error)
{
  auto sock = create_socket(
    host, port, 0, tcp_nodelay, std::move(socket_options), [&](socket_t sock, struct addrinfo& ai) -> bool {
      if (!intf.empty()) {
#ifdef USE_IF2IP
        auto ip = if2ip(intf);
        if (ip.empty()) {
      ...
      if (decompressor->is_valid()) {
        ContentReceiverWithProgress out = [&](const char* buf, size_t n, uint64_t off, uint64_t len) {
          return decompressor->decompress(buf, n,
                                          [&](const char* buf, size_t n) { return receiver(buf, n, off, len); });
        };
...
#if defined(_MSC_VER) && _MSC_VER < 1900
      n =
        static_cast<size_t>(_snprintf_s(&glowable_buf[0], glowable_buf.size(), glowable_buf.size() - 1, fmt, args...));
#else
...
{
  return detail::create_socket(host, port, socket_flags, tcp_nodelay_, std::move(socket_options),
...
inline socket_t ClientImpl::create_client_socket(Error& error) const
{
  ...
  return detail::create_client_socket(host_.c_str(), port_, tcp_nodelay_, socket_options_, connection_timeout_sec_,
                                      connection_timeout_usec_, interface_, error);
}
...
inline std::unique_ptr<Response> ClientImpl::send_with_content_provider(
  ...
  ContentProviderWithoutLength content_provider_without_length, const char* content_type, Error& error)
{
...
template<typename T>
inline bool process_server_socket_ssl(SSL* ssl, socket_t sock, size_t keep_alive_max_count,
...
inline Client::Client(const char* scheme_host_port, const std::string& client_cert_path,
                      const std::string& client_key_path)
{
