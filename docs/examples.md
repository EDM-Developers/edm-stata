...
The first example uses synthetic data, whereas the second example uses real data to look at the causal links between Chicago's temperature and crime rates.

The package's command keyword is `edm`, and should be immediately followed by the subcommand `explore` or `xmap`.

Note: A dataset must be declared as time-series or panel data by the tsset or xtset command prior to using the edm command, and time-series operators including `l.`, `f.`, `d.`, and `s.` can be used (the last for seasonal differencing).
