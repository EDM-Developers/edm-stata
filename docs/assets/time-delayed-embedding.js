update_manifold = function(refresh=true) {
	...
	for (let sliderID of sliderIDs) {
		...
		document.querySelectorAll(`.${sliderID}_choice`).forEach((elem) => elem.innerHTML = `${slider.value}`);

		// Update the current slider value (each time you drag the slider handle)
		slider.oninput = function(refresh=true) {
			document.querySelectorAll(`.${this.id}_choice`).forEach((elem) => elem.innerHTML = `${this.value}`)
			update_manifold()
		}
	}
